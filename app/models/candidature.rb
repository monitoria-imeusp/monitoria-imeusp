class Candidature < ActiveRecord::Base
  ...
  def self.all_first_options_for_request request
    where("semester_id = ? and course1_id = ?", request.semester.id, request.course.id).where.not(student_id: request.chosen_student_ids)
  end

  def self.all_nonfirst_options_for_request request
    where("semester_id = ? and (course1_id != ? and (course2_id = ? or course3_id = ? or course4_id = ?))", request.semester.id, request.course.id, request.course.id, request.course.id, request.course.id).where.not(student_id: request.chosen_student_ids)
  end
  ...
  def self.all_for_same_department_request request
    ...
    where.not(student_id: request.chosen_student_ids).find_by_sql("#{candidatures_in_same_department1.to_sql} UNION #{candidatures_in_same_department2.to_sql} UNION #{candidatures_in_same_department3.to_sql} UNION #{candidatures_in_same_department4.to_sql}")
  end

  def self.for_nusp_and_semester nusp, semester
    student = User.where(nusp: nusp).take.student
