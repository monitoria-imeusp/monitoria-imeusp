  ...
  context 'when logged in as admin' do
    ...
    describe 'create' do
      before :each do
        ...
        it { expect(Course).to receive(:new).with(any_args).and_return(@course) }
      end
    ...
    describe 'show' do
      ...
      context 'course does not exist' do
        before :each do
          it { expect(Course).to receive(:exists?).with(@id).and_return(false) }
          get :show, id: @id
        end
    ...
    describe 'update' do
      context 'a course that does not exist' do
        ...
        it { expect(response).to redirect_to course_path }
      end
    ...
    describe "destroy" do
      ...
      describe "destroy a course" do
        ...
        it "redirects to the courses list" do
          delete :destroy, {:id => @id}
          it { expect(response).to redirect_to(courses_url) }
        end
      end
    end
  end
